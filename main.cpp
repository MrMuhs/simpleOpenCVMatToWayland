...
// just hackish include the cpp
#include "wayland-shm.cpp"
...
{
	createWindowWayneland();

	Mat frame(capWidth, capHeight, CV_8UC3, Scalar(0,0,255));
	drawWindowWayneland(frame);

	while (waitKey(1) < 0)
	{
		drawWindowWayneland(frame);
		dispatchDisplayWayneland();
	}

	destroyWindowWayneland();
}

int JUSTDONOTHING/*_main*/(int argc, char** argv)
{
