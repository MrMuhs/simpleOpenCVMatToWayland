...
// https://raw.githubusercontent.com/eyelash/tutorials/master/wayland-shm.c
...
static void GetRawImage(cv::Mat _currentFrame, byte *data, int width, int height)
{
...
static void drawWindowWayneland(cv::Mat frame)
{
	/*
	int x, y;
	...
	*/
	...
	// just transform copy the frame to the shared memory
	GetRawImage(frame, window->data);
...
static void createWindowWayneland(int width, int height)
{
	display = wl_display_connect (NULL);
	struct wl_registry *registry = wl_display_get_registry (display);
	wl_registry_add_listener (registry, &registry_listener, NULL);
...
static void destroyWindowWayneland()
{
	delete_window(&window);
}

static void dispatchDisplayWayneland()
{
	wl_display_dispatch(display);
}